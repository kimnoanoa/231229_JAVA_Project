...
public class Collection01 {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * Collection 에서는 기본 자료형은 불가능. 클래스 자료형만 구성가능.
		 * 기본 자료형에 맞는 클래스 자료형을 사용하면 됨.
		 * int => Integer , String
		 * 나머지는 기본 자료형의 첫 글자만 대문자로 변환하면 클래스 자료형이 됨.
		 * long => Long ,  byte => Byte ,  boolean => Boolean....
		 * 
		 * 만약 클래스를 지정하지 않으면 Object가 자동설정됨.
		 * 
		...
		// remove (object) : object번지의 요소를 삭제
		list1.remove(0);
		System.out.println(list1);
		...
		// contain (object) : list에 값이 있는지 없는지 검사 트루/폴스로 리턴
		...
		// isEmpty () : 리스트 비어있는지 체크 비어있으면 true 아니면 false
		System.out.println(list1.isEmpty());
		
		// ArrayList number 를 생성 숫자를 저장하는 리스트
		// 1부터 10까지 채운 후 출력(향상포문)
		 ...
		 * 
		 * set 이나 map은 순서를 보장하지 않기 때문에 일반 for문을 사용할 수 없음.
		 * 향상된 for, Iterator 를 사용
		 * 순서와 상관없이 값을 가져올수 있는 구문을 사용해야 함.
		 * map 은 향상된 for, Iterator 둘다 직접 사용 불가능.
		 * 
		 * 
		 * 
		 * */
		...
		while (it.hasNext()) { // hasNext() : 다음 요소가 있는지 체크. t/f
			Integer tmp = it.next(); // next는 다음 요소 가져오기
		...
		//indexof(값) : 해당 값이 list에 있는지 확인하고  index반환. 없으면 -1리턴
